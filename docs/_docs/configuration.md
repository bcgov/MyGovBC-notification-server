...
To support this unsubscription method, a custom SMTP server needs to be setup separately from *NotifyBC*. The SMTP server should be able to convert emails sent to address *un-{subscriptionId}-{unsubscriptionCode}@{unsubscriptionEmailDomain}* to [unsubscribing API calls](../api-subscription/#delete-a-subscription-unsubscribing). This unsubscription email address is generated by *NotifyBC* and set in header *List-Unsubscribe* of all email notifications if following static config, which has no default value, is defined in */server/config.local.js*
...
[//]: <> (todo: update this para when smtp server is implemented.)
At the time of writing, the custom SMTP server is under development. Stay tuned.
